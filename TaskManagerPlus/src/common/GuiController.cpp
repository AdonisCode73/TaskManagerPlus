...
void GuiController::drawPage(WINDOW *win, Screen screen, const char *header) {
  ...
  case Screen::HOME: {
    ...
    for (int i = 0; i < numLines; ++i) {
      mvwprintw(win, ((m_rows - numLines) / 2) + i,
                (m_cols - static_cast<int>(strlen(banner[0]))) / 2, "%s",
                banner[i]);
    }
    break;
  }
    ...
    if (!systemStatus.cpuUsage.empty()) {
      {
        ...
        mvwprintw(win, middleRow, 2, "CPU Utilisation: %.2f%%",
                  systemStatus.cpuUsage.front());
      }
    }
    break;
  }
  ...
  case Screen::DISK: {
    mvwprintw(win, middleRow, 2, "Write Speed: %.2f KB/s",
              systemStatus.writeDisk.load());
    mvwprintw(win, middleRow + 1, 2, "Read Speed: %.2f KB/s",
              systemStatus.readDisk.load());
    if (!systemStatus.diskTime.empty()) {
      {
        std::lock_guard<std::mutex> lock(systemStatus.diskMutex);
        mvwprintw(win, middleRow + 2, 2, "Disk Utilisation: %.2f%%",
                  systemStatus.diskTime.front());
      }
    }

    mvwprintw(win, middleRow + 4, 2, "Total Space: %.2f GB",
              systemStatus.totalDisk.load());
    mvwprintw(win, middleRow + 5, 2, "Available Space: %.2f GB",
              systemStatus.availDisk.load());
    ...
    if (!systemStatus.gpuUsage.empty()) {
      {
        ...
        mvwprintw(win, middleRow, 2, "GPU Usage: %.2f%%",
                  systemStatus.gpuUsage.front());
      }
    }

    if (systemStatus.gpuType == GpuType::AMD) {
      mvwprintw(win, middleRow + 1, 2, "GPU Temperature: %.1f%°c",
                systemStatus.gpuTemperature.load());
      mvwprintw(win, middleRow + 5, 2, "GPU Clock Speed: %.2f Mhz",
                systemStatus.gpuClockSpeed.load());
    } else if (systemStatus.gpuType == GpuType::NVIDIA) {
      mvwprintw(win, middleRow + 1, 2, "GPU Memory Controller: %.2f%%",
                systemStatus.memControllerUsage.load());
    }

    mvwprintw(win, middleRow + 3, 2, "Total VRAM: %.2f GB",
              systemStatus.vramTotalMemory.load());
    mvwprintw(win, middleRow + 4, 2, "Available VRAM: %.2f GB",
              systemStatus.vramAvailMemory.load());
    break;
  }
    ...
    if (!systemStatus.memoryUsage.empty()) {
      {
        ...
        mvwprintw(win, middleRow, 2, "RAM Usage: %.2f%%",
                  systemStatus.memoryUsage.front());
      }
    }

    mvwprintw(win, middleRow + 2, 2, "Total RAM: %.2f GB",
              systemStatus.ramTotalMemory.load());
    mvwprintw(win, middleRow + 3, 2, "Available RAM: %.2f GB",
              systemStatus.ramAvailMemory.load());
    wrefresh(win);
    break;
  }
    ...
    if (!systemStatus.networkUsage.empty()) {
      {
        ...
        mvwprintw(win, middleRow, 2, "Network Usage: %.2f%%",
                  systemStatus.networkUsage.front());
      }
    }

    mvwprintw(win, middleRow + 2, 2, "Network Bytes Sent: %.2f Kbps",
              systemStatus.sendNetwork.load());
    mvwprintw(win, middleRow + 3, 2, "Network Bytes Received: %.2f Kbps",
              systemStatus.receiveNetwork.load());
    break;
  }
...
void GuiController::drawBaseLayout(WINDOW *win, const char *header) {
  ...
  const char *navigationFooter =
      "Press left or right arrow keys to traverse the screens";
  const char *exitFooter = "Press enter to exit the application";

  mvwprintw(win, 0, (m_cols - static_cast<int>(strlen(header))) / 2, "%s",
            header);
  mvwprintw(win, m_rows - 1,
            (m_cols - static_cast<int>(strlen(navigationFooter))) / 2, "%s",
            navigationFooter);
  mvwprintw(win, m_rows - 2,
            (m_cols - static_cast<int>(strlen(exitFooter))) / 2, "%s",
            exitFooter);
  ...
  if (m_currentScreen != Screen::HOME) {
    ...
    drawGraphBox(win, startY, startX, graphHeight, graphWidth,
                 "Utilisation Graph");
  }
  wrefresh(win);
}

void GuiController::drawGraphBox(WINDOW *win, int startY, int startX,
  ...
  mvwprintw(graphBox, 0, (width - static_cast<int>(strlen(title))) / 2, "%s",
            title);
...
void GuiController::renderGraph(WINDOW *win, const std::deque<double> data,
  ...
  int dataPoints =
      static_cast<int>(std::min(data.size(), static_cast<size_t>(m_maxBars)));
...
const char *GuiController::getHeader(HeaderType type) const {
  ...
  case Screen::NETWORK:
    return (type == HeaderType::Main) ? "Network Monitor"
                                      : "Network Statistics";
